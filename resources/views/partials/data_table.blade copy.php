<!-- [file name: data_table.blade.php] -->
                    ...
                    @if($hasActions)
                        <td>
                            @if(isset($column['action']))
                                {!! $column['action'] !!}
                            @elseif($editModalId)
                                <button class="btn" onclick="openEditModal('{{ $editModalId }}', {{ json_encode($item) }})">
                                <x-icon name="edit" /> 
                                </button>
                            @endif
                        </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updatePagination() {
            ...
            prevButton.style.disable = currentPage > 1 ? 'false' : 'true';
            nextButton.style.disable = currentPage < totalPages ? 'false' : 'true';
